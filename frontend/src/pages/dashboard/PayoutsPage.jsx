...
const PayoutsPage = () => {
  ...
  const getStatusConfig = (status) => {
    switch (status) {
      case 'completed': return { icon: CheckCircle, color: 'text-green-400', bg: 'bg-green-500/10', label: t('payouts.status.completed') }
      case 'pending': return { icon: Clock, color: 'text-yellow-400', bg: 'bg-yellow-500/10', label: t('payouts.status.processing') }
      case 'rejected': return { icon: XCircle, color: 'text-red-400', bg: 'bg-red-500/10', label: t('payouts.status.rejected') }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">{t('payouts.subtitle')}</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-dark-100/80 backdrop-blur-xl rounded-xl p-6 border border-white/5 hover:border-green-500/30 transition-all duration-300 group">
          <div className="flex items-center justify-between mb-2">
            <span className="text-gray-400">{t('payouts.availableBalance')}</span>
          ...
          <p className="text-sm text-green-400 mt-2">{t('payouts.readyToWithdraw')}</p>
        </div>
        <div className="bg-dark-100/80 backdrop-blur-xl rounded-xl p-6 border border-white/5 hover:border-blue-500/30 transition-all duration-300 group">
          <div className="flex items-center justify-between mb-2">
            <span className="text-gray-400">{t('payouts.totalWithdrawn')}</span>
          ...
          <p className="text-sm text-gray-500 mt-2">{payouts.filter(p => p.status === 'completed').length} {t('payouts.completedWithdrawals')}</p>
        </div>
        <div className="bg-dark-100/80 backdrop-blur-xl rounded-xl p-6 border border-white/5 hover:border-yellow-500/30 transition-all duration-300 group">
          <div className="flex items-center justify-between mb-2">
            <span className="text-gray-400">{t('payouts.inProgress')}</span>
            <div className="p-1.5 rounded-lg bg-yellow-500/10">
              <Clock size={18} className="text-yellow-400" />
            </div>
          </div>
          <p className="text-3xl font-bold text-yellow-400">${pendingAmount.toLocaleString()}</p>
          <p className="text-sm text-gray-500 mt-2">{t('payouts.pendingProcessing')}</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-dark-200/50 rounded-xl p-4 border border-white/5 hover:border-blue-500/30 transition-all duration-300 cursor-pointer group">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-medium text-white group-hover:text-blue-400 transition-colors">{t('payouts.bankTransfer')}</h4>
                <p className="text-sm text-gray-400">{t('payouts.bankTransferTime')}</p>
              </div>
            </div>
          </div>
          <div className="bg-dark-200/50 rounded-xl p-4 border border-white/5 hover:border-orange-500/30 transition-all duration-300 cursor-pointer group">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-medium text-white group-hover:text-orange-400 transition-colors">{t('payouts.cryptoUsdt')}</h4>
                <p className="text-sm text-gray-400">{t('payouts.cryptoTime')}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-blue-500/10 backdrop-blur-xl rounded-xl border border-blue-500/20 p-4 flex items-start gap-3">
        ...
        <div>
          <h4 className="font-medium text-white mb-1">{t('payouts.withdrawalInfo')}</h4>
          <p className="text-sm text-gray-400">
            {t('payouts.withdrawalInfoDesc')}
          </p>
        </div>
      </div>
      ...
      <div className="bg-dark-100/80 backdrop-blur-xl rounded-xl border border-white/5 overflow-hidden">
        <div className="p-4 border-b border-white/5">
          <h3 className="font-semibold text-white">{t('payouts.withdrawalHistory')}</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-dark-200/50">
              <tr className="text-xs text-gray-500 uppercase">
                <th className="px-6 py-4 text-left font-medium">{t('payouts.tableHeaders.id')}</th>
                <th className="px-6 py-4 text-left font-medium">{t('payouts.tableHeaders.method')}</th>
                <th className="px-6 py-4 text-left font-medium">{t('payouts.tableHeaders.requestDate')}</th>
                <th className="px-6 py-4 text-left font-medium">{t('payouts.tableHeaders.processedDate')}</th>
                <th className="px-6 py-4 text-left font-medium">{t('payouts.tableHeaders.status')}</th>
                <th className="px-6 py-4 text-right font-medium">{t('payouts.tableHeaders.amount')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {payouts.map(payout => {
                ...
                const dateLocale = i18n.language === 'ar' ? 'ar-MA' : i18n.language === 'en' ? 'en-US' : 'fr-FR'
                return (
                  <tr key={payout.id} className="hover:bg-dark-200/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-white capitalize">{payout.method === 'bank' ? t('payouts.methodLabels.bank') : t('payouts.methodLabels.crypto')}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-gray-400">
                      {new Date(payout.date).toLocaleDateString(dateLocale)}
                    </td>
                    <td className="px-6 py-4 text-gray-400">
                      {payout.processedDate ? new Date(payout.processedDate).toLocaleDateString(dateLocale) : '-'}
                    </td>
                    <td className="px-6 py-4">
