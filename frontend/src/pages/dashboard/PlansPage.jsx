...
const PlansPage = () => {
  ...
  return (
    <div className="space-y-8 -m-4 md:-m-6">
      ...
      {/* AI Tiers Section */}
      <section className="relative py-8 overflow-hidden mx-4 md:mx-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 lg:gap-4">
            {Object.entries(AI_TIERS).reverse().map(([key, tier], index) => {
              ...
              return (
                <div
                  key={key}
                  className="group flex items-center gap-3 px-4 py-2 glass-card rounded-xl transition-all duration-300 ease-out
      ...
      {models.length > 1 && (
        <div className="mx-4 md:mx-6">
          <h3 className="text-lg font-semibold text-white mb-4">Choisissez votre modèle de challenge</h3>
              ...
              <button
                ...
                className={`relative p-6 rounded-xl border-2 transition-all text-left ${
                  selectedModel?.id === model.id
                    ...
                    : 'border-dark-200 bg-dark-100 hover:border-primary-500/30'
                }`}
              >
                {model.is_popular && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-gradient-to-r from-orange-500 to-orange-600 text-white text-xs font-bold rounded-full flex items-center gap-1">
                    <Flame size={12} />
                    POPULAIRE
                  </span>
                )}
                <div className={`w-12 h-12 rounded-xl mb-4 flex items-center justify-center`} style={{ backgroundColor: (model.badge_color || '#22c55e') + '20' }}>
                  <Zap size={24} style={{ color: model.badge_color || '#22c55e' }} />
                </div>
                <h3 className="text-lg font-bold text-white mb-1">{model.display_name}</h3>
                <p className="text-sm text-gray-400 mb-4">{model.description || `Évaluation en ${model.phases} phases`}</p>
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2 text-gray-400">
                    <CheckCircle2 size={16} className="text-green-500" />
                    <span>Objectif: {model.phase1_profit_target}%</span>
                  </div>
                  <div className="flex items-center gap-2 text-gray-400">
                    <CheckCircle2 size={16} className="text-green-500" />
                    <span>Perte max: {model.max_daily_loss}%</span>
                  </div>
                  <div className="flex items-center gap-2 text-gray-400">
                    <CheckCircle2 size={16} className="text-green-500" />
                    <span>Partage: {model.default_profit_split}%</span>
                  </div>
                </div>
              </button>
            ))}
