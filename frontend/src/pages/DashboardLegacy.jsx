...
// Symbol categories for quick selection
...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Also refresh challenge context
      refetchChallenge()
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-dark-300 py-6">
  ...
  // Plan badge colors
  const planColors = {
    starter: { bg: 'bg-blue-500', text: 'text-blue-500', light: 'bg-blue-500/10' },
    pro: { bg: 'bg-primary-500', text: 'text-primary-500', light: 'bg-primary-500/10' },
    elite: { bg: 'bg-purple-500', text: 'text-purple-500', light: 'bg-purple-500/10' }
  }
  const planColor = planColors[challenge.plan_type] || planColors.starter
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-dark-300">
      {/* Top Bar */}
      <div className="bg-white dark:bg-dark-100 border-b border-gray-200 dark:border-dark-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className={`p-2 rounded-lg ${phaseColor.light}`}>
                {challenge.is_funded ? <Trophy className={phaseColor.text} size={24} /> :
                 challenge.phase === 'trial' ? <Clock className={phaseColor.text} size={24} /> :
                 challenge.phase === 'verification' ? <TrendingUp className={phaseColor.text} size={24} /> :
                 <Target className={phaseColor.text} size={24} />}
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-xl font-bold text-gray-900 dark:text-white">
                    {challenge.phase_display || challenge.phase?.toUpperCase()}
                  </h1>
                  {challenge.is_funded && (
                    <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-green-500/10 text-green-500 flex items-center gap-1">
                      <Star size={12} />
                      Funded
                    </span>
                  )}
                  {challenge.is_trial && !challenge.is_trial_expired && (
                    <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-blue-500/10 text-blue-500 flex items-center gap-1">
                      <Clock size={12} />
                      {challenge.trial_days_remaining}j restants
                    </span>
                  )}
                  <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${
                    challenge.status === 'active' || challenge.status === 'funded' ? 'bg-green-500/10 text-green-500' :
                    challenge.status === 'passed' ? 'bg-blue-500/10 text-blue-500' :
                    'bg-red-500/10 text-red-500'
                  }`}>
                    {challenge.status === 'active' ? 'Actif' :
                     challenge.status === 'funded' ? 'Funde' :
                     challenge.status === 'passed' ? 'Reussi' : 'Echoue'}
                  </span>
                </div>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Plan {challenge.plan_type?.toUpperCase()} - Depuis {new Date(challenge.start_date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' })}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              {challenge.is_funded && challenge.withdrawable_profit > 0 && (
                <Link
                  to="/dashboard/withdraw"
                  className="flex items-center gap-2 px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg font-medium transition-colors"
                >
                  <Banknote size={18} />
                  Retirer ${challenge.withdrawable_profit?.toLocaleString()}
                </Link>
              )}
              <MarketStatus />
              <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {/* Balance */}
          <div className="bg-white dark:bg-dark-100 rounded-xl p-4 border border-gray-100 dark:border-dark-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Balance</span>
          ...
          <div className="bg-white dark:bg-dark-100 rounded-xl p-4 border border-gray-100 dark:border-dark-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Equity</span>
          ...
          <div className="bg-white dark:bg-dark-100 rounded-xl p-4 border border-gray-100 dark:border-dark-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Objectif Profit</span>
            ...
            <div className="flex items-end gap-2">
              ...
              <p className="text-sm text-gray-400 mb-1">/ {profitTarget}%</p>
            </div>
            <div className="mt-2 h-1.5 bg-gray-100 dark:bg-dark-200 rounded-full overflow-hidden">
              <div
          ...
          <div className="bg-white dark:bg-dark-100 rounded-xl p-4 border border-gray-100 dark:border-dark-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Drawdown</span>
              <Activity size={16} className={stats.drawdown > stats.maxDrawdown * 0.7 ? 'text-red-500' : 'text-gray-400'} />
            </div>
            <div className="flex items-end gap-2">
              <p className={`text-2xl font-bold ${stats.drawdown > stats.maxDrawdown * 0.7 ? 'text-red-500' : 'text-gray-900 dark:text-white'}`}>
                {stats.drawdown.toFixed(1)}%
              </p>
              <p className="text-sm text-gray-400 mb-1">/ {stats.maxDrawdown}% max</p>
            </div>
            <div className="mt-2 h-1.5 bg-gray-100 dark:bg-dark-200 rounded-full overflow-hidden">
              <div
        ...
        {/* Trading Stats Row */}
        <div className="bg-white dark:bg-dark-100 rounded-xl border border-gray-100 dark:border-dark-200 p-4 mb-6">
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4 md:gap-6">
            ...
            <div className="text-center md:text-left">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Best Trade</p>
              <p className="text-lg font-bold text-green-500">+${stats.bestTrade.toFixed(2)}</p>
            </div>
            <div className="text-center md:text-left">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Worst Trade</p>
              <p className="text-lg font-bold text-red-500">${stats.worstTrade.toFixed(2)}</p>
            </div>
            <div className="text-center md:text-left">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Avg Win</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Chart */}
            <div className="bg-white dark:bg-dark-100 rounded-xl border border-gray-100 dark:border-dark-200 overflow-hidden">
              <div className="p-4 border-b border-gray-100 dark:border-dark-200">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <CircleDot size={16} className="text-green-500" />
                    <span className="font-semibold text-gray-900 dark:text-white">{selectedSymbol}</span>
                  </div>
                  <div className="flex gap-1 bg-gray-100 dark:bg-dark-200 p-1 rounded-lg">
            ...
            {/* Open Positions */}
            {openTrades.length > 0 && (
              <div className="bg-white dark:bg-dark-100 rounded-xl border border-gray-100 dark:border-dark-200">
                <div className="p-4 border-b border-gray-100 dark:border-dark-200 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Activity size={18} className="text-green-500" />
                    <span className="font-semibold text-gray-900 dark:text-white">Positions Ouvertes</span>
                ...
                <div className="divide-y divide-gray-100 dark:divide-dark-200">
                  {openTrades.map(trade => {
                    ...
                    return (
                      <div key={trade.id} className="p-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-dark-200/50 transition-colors">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                              ...
                              {pnlInfo && <span className="text-gray-400"> → ${pnlInfo.current_price.toFixed(2)}</span>}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-4">
              ...
              <div className="bg-white dark:bg-dark-100 rounded-xl border border-gray-100 dark:border-dark-200">
                <div className="p-4 border-b border-gray-100 dark:border-dark-200 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-400">({closedTrades.length} trades)</span>
                  </div>
                </div>
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-dark-200">
                      {closedTrades.slice(0, 10).map(trade => (
                        <tr key={trade.id} className="hover:bg-gray-50 dark:hover:bg-dark-200/30 transition-colors">
                          ...
                          <td className="px-4 py-3 text-gray-600 dark:text-gray-400">${Number(trade.entry_price).toFixed(2)}</td>
                          <td className="px-4 py-3 text-gray-600 dark:text-gray-400">${Number(trade.exit_price).toFixed(2)}</td>
